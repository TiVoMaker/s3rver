  ...
  dumpLogs() {
    if (this.logLines.length > 0) {
      ...
      this.initialize(() => {
        ...
        let pos = 0;
        console.log(`==> start writing ${buffer.length} logfile`);
        let rs = new stream.Readable({
          read(size) {
            console.log(`==> entering next read loop`);
            while (pos < buffer.length) {
              let left = buffer.length - pos;
              let tosend = (left > size ? size : left);
              console.log(`==> can send ${size}, pushing ${tosend} bytes starting at ${pos}`);
              if (!this.push(buffer.slice(pos, tosend))) {
                pos += tosend;
                console.log(`==> flow controlled, start updated to ${pos}`);
                return;
              }
              pos += tosend;
            }
            console.log(`==> think we sent ${pos} total bytes`);
            this.push(null);
          }
        });
        rs.headers = {
