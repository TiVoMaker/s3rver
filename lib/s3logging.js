...
class S3Logger {
  ...
  constructor(rootDirectory, appLogger, logBucket, logPrefix, logInterval, bucketsToLog) {
    ...
    if (this.bucketsToLog.includes(logBucket)) {
      this.appLogger.info('Log bucket is included in buckets to log, it will be ignored');
      this.bucketsToLog = this.bucketsToLog.filter(b => b === this.logBucket);
    }
  ...
  logEntry(m) {
    ...
    if (!this.bucketsToLog.includes(bucket)) {
      if (bucket === this.logBucket) {
        // Implicit log flush for testing
        if (m.method.toLowerCase() === 'get') {
          if (urlParts.pathname.split('/').length === 2) {
              this.appLogger.info('implicit flush on list buckets');
              this.dumpLogs();
          }
        }
      }
