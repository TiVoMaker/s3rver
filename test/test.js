...
describe("S3rver Tests", function() {
  ...
  afterEach("Close server", function*() {
    yield Promise.resolve(server.close(() => {}));
  });
...
describe("Test AWS style access logging", function() {
  ...
  function ingestLogs() {
    ...
    return s3Client.listObjects({
      ...
      // First call triggers a flush of the logs, wait a bit to let
      // them get out.
  ...
  before("Start s3rver and make a test bucket", function*() {
    yield thunkToPromise(done => {
      s3rver = new S3rver({
        port: 5694,
        hostname: "localhost",
    ...
    s3Client = new AWS.S3({
      accessKeyId: "123",
      secretAccessKey: "abc",
      endpoint: util.format("http://%s:%d", 'localhost', 5694),
      sslEnabled: false,
      s3ForcePathStyle: true
    });
    yield s3Client.createBucket({ Bucket: testBucket }).promise();
  });

  after("Shut down the server", function*() {
    yield thunkToPromise(done => s3rver.close(done));
  });

  beforeEach("Clean out old logs", function*() {
    let data;
    try {
      data = yield s3Client.listObjects({
        Bucket: logBucket,
        Prefix: logPrefix
      }).promise();
    } catch (err) {
  ...
  it("should create an object, fetch it, and see the log entries", function*() {
    ...
    yield s3Client.getObject({
      Bucket: testBucket,
      Key: testKey
    }).promise();
    let logLines = yield ingestLogs();
    expect(logLines.length).to.equal(2);
    expect(logLines[0].indexOf('REST.PUT.OBJECT')).to.be.greaterThanOrEqual(0);
    expect(logLines[0].indexOf(testBucket)).to.be.greaterThanOrEqual(0);
    expect(logLines[0].indexOf(testKey)).to.be.greaterThanOrEqual(0);
    expect(logLines[1].indexOf('REST.GET.OBJECT')).to.be.greaterThanOrEqual(0);
    expect(logLines[1].indexOf(testBucket)).to.be.greaterThanOrEqual(0);
    expect(logLines[1].indexOf(testKey)).to.be.greaterThanOrEqual(0);
  });

  it("should handle logging the query string properly", function*() {
    yield s3Client.putObject({
      Bucket: testBucket,
      Key: testKey,
      Body: testContents
    ...
    let logLines = yield ingestLogs();
    expect(logLines.length).to.equal(2);
    expect(logLines[1].indexOf(`GET /${testBucket}`)).to.be.greaterThanOrEqual(0);
    const query = new RegExp(`GET /${testBucket}/${testKey}\\?([^ ]+)`).exec(logLines[1]);
    expect(query).to.exist();
    const qparts = qs.parse(query[1]);
    expect(qparts).to.exist();
    expect(qparts['x-r']).to.exist();
    expect(qparts['x-r']).to.equal('Testing!');
  });

  it("should log the copy operation properly", function*() {
    yield s3Client.putObject({
      Bucket: testBucket,
      Key: testKey,
      Body: testContents
    ...
    expect(logLines[1].indexOf(`GET /${testBucket}/${testKey}`)).to.be.greaterThanOrEqual(0);
    expect(logLines[1].indexOf('REST.COPY.OBJECT')).to.be.greaterThanOrEqual(0);
    expect(logLines[2].indexOf(`PUT /${testBucket}/${tkey}`)).to.be.greaterThanOrEqual(0);
  });
